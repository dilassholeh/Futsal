...
$nominal_bayar = ($payment_type == "lunas") 
    ? $booking['total'] 
    : ($booking['total'] / 2);
...
$stmt = $conn->prepare("
    ...
    VALUES (?, ?, ?, ?, ?, 'transfer', NOW())
");
$stmt->bind_param("ssdss", $id_transaksi, $user_id, $nominal_bayar, $filename, $status_pembayaran);
$stmt->execute();
$stmt->close();
...
$userNotif = $conn->prepare("
    ...
    VALUES (?, ?, ?, ?, 'baru', NOW())
");
$userNotif->bind_param("ssss", $user_id, $id_transaksi, $judul_user, $pesan_user);
$userNotif->execute();
$userNotif->close();
