...
$totalEvent = mysqli_fetch_assoc(mysqli_query($conn, "
SELECT COUNT(*) AS total
FROM event
WHERE CURDATE() BETWEEN tanggal_mulai AND tanggal_berakhir
"))['total'] ?? 0;
...
$qPendapatan = mysqli_fetch_assoc(mysqli_query($conn, "
SELECT COALESCE(SUM(jumlah_dibayar),0) AS total
FROM transaksi
WHERE status_pembayaran IN ('menunggu_konfirmasi','lunas')
"));
$totalPendapatan = $qPendapatan['total'] ?? 0;

$qHari = mysqli_query($conn, "
SELECT COALESCE(SUM(jumlah_dibayar),0) AS total
FROM transaksi
WHERE DATE(created_at)=CURDATE()
AND status_pembayaran IN ('menunggu_konfirmasi','lunas')
");
